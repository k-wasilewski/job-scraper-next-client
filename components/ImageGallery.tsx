...
export const ImageGallery = (props: ImageGalleryProps) => {
    ...
    useLayoutEffect(() => {
        window.addEventListener('click', (e: MouseEvent) => {
            ...
            if (targetElement.tagName !== 'BUTTON' && imgRef.current && 
            ...
            {images && images.length ? images.map((image, i) => {
                if (i===currentIndex) return (
                    <div key={`image-${i}`} style={{backgroundColor: 'rgba(0, 0, 0, 0.7)', minHeight: '100vh', maxWidth: '100vw' }}>
                        ...
                        <button className='btn btn-light my-3' style={{margin: '0 auto', display: 'table'}} onClick={() => onClose()}>Close</button>

                        <button onClick={setPrevImage} style={{ background: 'none', border: 'none' }}>
                            <img alt='Previous job' src={arrowLeft.src} ref={arrowLeftRef} style={{position: 'fixed', width: '50px', filter: 'invert(100%)', top: '50%', left: '1%', cursor: 'pointer'}} />
                        </button>
                        <button onClick={setNextImage} style={{ background: 'none', border: 'none' }}>
                            <img alt='Next job' src={arrowLeft.src} ref={arrowRightRef} style={{position: 'fixed', width: '50px', filter: 'invert(100%)', top: '50%', right: '1%', cursor: 'pointer', transform: 'scaleX(-1)'}} />
                        </button>
